...
const CategoryAddForm = ({ itemEdit }) => {
  ...
  const initVal = {
   category_title: itemEdit ? itemEdit.movies_title : '',
   
   category_title_old: itemEdit ? itemEdit.movies_title : '',
}
        ...
        >
          ...
          {(props) => {
            ...
            <div className='settings-action flex justify-end gap-3 my-3'>
                ...
                <button className='btn btn-cancel w-[80px] flex justify-center'onClick={()=> dispatch(setIsAdd(false))}>Cancel</button>
            </div>
        </Form>
         );
        }}
