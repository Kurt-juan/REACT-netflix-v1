...
const SeriesSliderTopTen = ({title="",isTopTen=true, category}) => {
  ...
  return (
   ...
   <img src={`${devImgPath}/${item.topseries_image}`} alt="" className='w-[331px]
   ...
   h-[441px] z-40 ">
       ...
       <p className='flex items-center mb-5'>{item.topseries_genre}<Dot/>{item.topseries_year} <Dot/>{item.topseries_duration} </p>
       <h4 >Summary</h4>
   ...
   {isTopTen &&
   <p className='absolute -bottom-3 -left-2 text-[200px] mb-0 leading-none font-black text-accent counter group-hover:opacity-0'>{item.topseries_ranking}</p>
   }
</div>
            ))}
